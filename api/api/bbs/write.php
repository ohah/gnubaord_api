...
trait write {
  public function write($bo_table, $wr_id = null, $w = '') {
    ...
    if ($w == '') {
      if ($wr_id) {
        $this->alert('글쓰기에는 \$wr_id 값을 사용하지 않습니다.', G5_BBS_URL.'/board.php?bo_table='.$bo_table);
      }
      ...
      $sql = " SELECT count(*) as cnt FROM {$write_table}
                  ...
                  AND wr_num = ?
                  AND wr_is_comment = 0";
      $row = $this->sql_fetch($sql, [$reply.'%', $write['wr_id'], $write['wr_num']]);
      if ($row['cnt'] && !$is_admin)
      ...
      $sql = " SELECT count(*) as cnt FROM {$write_table}
                  ...
                  AND mb_id <> ?
                  AND wr_is_comment = 1 ";
      $row = $this->sql_fetch($sql, [$wr_id, $member['mb_id']]);
    ...
    if (!empty($group['gr_use_access'])) {
      ...
      if ($is_admin == 'super' || $group['gr_admin'] === $member['mb_id'] || $board['bo_admin'] === $member['mb_id']) {
        ...
        // 그룹접근
        $sql = " select gr_id from {$g5['group_member_table']} where gr_id = ? and mb_id = ? ";
        $row = $this->sql_fetch($sql, [$board['gr_id'], $member['mb_id']]);
        if (!$row['gr_id'])
