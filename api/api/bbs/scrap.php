...
trait scrap {
  public function scrap() {
    ...
    for ($i=0; $i<count($result); $i++) {
      ...
      // 게시판 제목
      $sql2 = " select bo_subject from {$g5['board_table']} where bo_table = ?";
      $row2 = $this->sql_fetch($sql2, [$row['bo_table']]);
  ...
  public function scrap_popin_update($bo_table, $wr_id) {
    ...
    and bo_table = ?
    and wr_id = ?";
    $row = sql_fetch($sql. [$member['mb_id'], $bo_table, $wr_id]);
    if ($row['cnt']) {
    ...
    if ($wr_content && ($member['mb_level'] >= $board['bo_comment_level'])) {
      ...
      if ($wr['wr_id']) {
        ...
        $sql = " select max(wr_comment) as max_comment from $write_table
                    where wr_parent = ? and wr_is_comment = ?";
        ...
        // 코멘트 1 증가
        $this->sql_query(" update {$g5['board_table']} set bo_count_comment = bo_count_comment + 1 where bo_table = ?",[$bo_talbe]);
    ...
    $sql = " insert into {$g5['scrap_table']} ( mb_id, bo_table, wr_id, ms_datetime ) values ( ?, ?, ?, ? ) ";
    $this->sql_query($sql, [$member['mb_id'], $bo_table, $wr_id, G5_TIME_YMDHIS]);

    $sql = " update `{$g5['member_table']}` set mb_scrap_cnt = ? where mb_id = ?";
    $this->sql_query($sql, [$this->get_scrap_totals($member['mb_id']), $member['mb_id']]);
